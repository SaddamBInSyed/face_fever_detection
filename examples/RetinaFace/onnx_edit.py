            ...
            if (noutput == name) and (n.name not in node_input_names):
                ...
                if n.name in node_map.keys():
                    for ninput in node_map[n.name].input:
                        ...
                        node_input_names = traceDependentNodes(graph, ninput, node_input_names, node_map,
                                                               initializer_map)
...
def onnx_edit(input_model, output_model, new_input_node_names, input_shape_map, new_output_node_names, output_shape_map,
              verify):
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", default="models/mxnet_exported_R50.onnx" ,help="input onnx model")
    parser.add_argument("output", default="models/mxnet_exported_R50_edit.onnx", help="output onnx model")
    parser.add_argument("--inputs",
                        help="comma separated model input names appended with shapes, e.g. --inputs <nodename>[1,2,3],<nodename1>[1,2,3] ")
    parser.add_argument("--outputs",
                        help="comma separated model output names appended with shapes, e.g. --outputs <nodename>[1,2,3],<nodename1>[1,2,3] ")
