...
def main():
    ...
    g_dumpPath = '/media/481D35AA4108CBB0/fever_det/'
        ...
        while True:
            ...
            if g_showPic:
                ...
                if g_calib_black_body:
                    A,B = calib_black_body(im=im, im_raw=im_raw,num_of_body=2)
                ...
                # im_plot = im
                im_plot = cv2.applyColorMap(im_plot,2) # 2 is type of color map
                ...
                if rect_est is not None:
                    ...
                    ref_temp_raw = np.mean(im_raw[int(top+hh*dh):int(bottom-hh*dh), int(left+hh*dw):int(right-hh*dw), 0])
                    print('black body found!, {}'.format(ref_temp_raw))
                    status_text = 'black body found'
                    color = (0, 255, 0)
                    cv2.rectangle(im_plot, (2 * int(left+hh*dw), 2 * int(top+hh*dh) ), (2 * int(right-hh*dw), 2 * int(bottom-hh*dh)), color, 3)
                    cv2.rectangle(im_plot, (2 * left, 2 * top), (2 * right, 2 * bottom), color, 3)


                else:
                    ref_temp_raw = np.median(np.sort(im_raw[:, :, 0].reshape([-1]))[-100:])
                ...
                ref_temp = ref_temp_raw * A +B
                print(ref_temp_raw)
                print(ref_temp)


                #cv2.rectangle(im_plot, (2 * left, 2 * top), (2 * right, 2 * bottom), color, 3)
                # cv2.putText(im_plot, 'ref', (2 * left, 2 * top), (2 * right, 2 * bottom - 20), font, 2, color, 3)
                ...
                if faces is not None and len(faces) > 0:
                    ...
                    for i in range(faces.shape[0]):
                        ...
                        w = box_draw[2] - box_draw[0]
                        h = box_draw[3] - box_draw[1]
                        ...
                        cv2.rectangle(im_plot, (hor_start, ver_start), (hor_end, ver_end), (255, 255, 255), 2)
                ...
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
            if g_dumpFiles:
                frame_name = os.path.join(g_dumpPath, "frame_{}.jpg".format(thermapp.imageId))
...
def detect_rectangle(img):
    ...
    for cnt in contours:
        ...
        if area > 2200 and area < 4200:
            ...
            est_polyline = cv2.approxPolyDP(cnt, 0.009 * cv2.arcLength(cnt, True), True)  # [top left, bottom left, bottom right, top right]
